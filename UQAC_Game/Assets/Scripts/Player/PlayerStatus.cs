...
public class PlayerStatus : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [Tooltip("The current Task Progession of our player")]
    public float ProgessionTasksPerso = 0f;
    ...
    void Start()
    {
        globalScore = GameObject.Find("Global Score").GetComponent<GlobalScore>();
    }
    ...
    private void SetProgessionTasksGlobal(float value)
    {
        ...
        if (globalScore != null) globalScore.SetGlobalScore((int)this.ProgessionTasksGlobal);
    }

    private float GetProgessionTasksGlobal()
    {
        if (globalScore != null) this.ProgessionTasksGlobal = globalScore.GetGlobalScore();
